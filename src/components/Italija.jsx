...
const Italija = () => {
  const [podatci, setPodatci] = useState(localStorage.getItem("italijaPodatci") || "");
  const initialDokumentacijaState = () => {
    return Array.from({ length: 15 }, (_, index) => {
      const savedData = localStorage.getItem(`italijaDokumentacija${index}`);
  ...
  useEffect(() => {
    localStorage.setItem("italijaPodatci", podatci);
  }, [podatci]);

  useEffect(() => {
    dokumentacija.forEach((item, index) => {
      localStorage.setItem(`italijaDokumentacija${index}`, JSON.stringify(item));
    });
  }, [dokumentacija]);
  ...
  return (
    <div className="container">
      <h1 style={{ color: 'blue' }}>Italija</h1> 
